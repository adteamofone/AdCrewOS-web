...
## Inline guardrail editing + white-label Agency PDFs (2026-08-13, Charles's request)

- Thresholds were only settable during onboarding while the detail page pointed at
  Settings (which had no such controls). Per Charles: adjust them **in the cockpit**.
  Added `ThresholdStepper` (client) — ▲/▼ arrows step target/pause/scale by 0.1 with
  optimistic UI and a debounced save (rapid clicks collapse into one write) via a new
  `updateThreshold` server action (ownership-checked, 0.1–1,000,000 clamp, 2-dp round).
  Wired into every cockpit account card (compact guardrails row) and the account detail
  Guardrails card; the "adjust in Settings" copy now explains the arrows instead.
- Agency PDFs are white-labeled: footer no longer says "Generated by AdCrewOS"
  (now "Performance Report"), and an Agency report with no uploaded logo renders a
  blank header slot rather than falling back to the AdCrewOS wordmark. Solo PDFs keep
  full AdCrewOS branding.
